  ...
  const handleTranslate = useCallback(async () => {
    ...
    try {
      for (let i = 0; i < untranslated.length; i += batchSize) {
        ...
        setEntries(prev =>
          prev.map(entry => {
            const result = data.translations.find(
              (t: { index: number }) => t.index === entry.index
            )
            if (result) {
              return { ...entry, msgstr: result.translated, status: 'translated' as const }
            }
            return entry
          })
        )
        ...
        {[
          ...
          return (
            <div key={s.key} className="flex items-center gap-2">
              ...
              <span className={isActive ? 'text-white font-medium' : 'text-white/30'}>
                {s.label}
              </span>
      ...
      {step === 'upload' && (
        <div className="glass-card p-8">
          <div className="max-w-md mx-auto space-y-6">
                ...
                {file ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-white/50">
                      Drop .po or .pot file here or click to browse
                    </p>
            ...
            {/* Demo option */}
            <div className="text-center">
              <p className="text-sm text-white/30 mb-2">No .po file handy?</p>
              <button
                onClick={() => {
                  setEntries([
          ...
          <div className="glass-card p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-white font-medium">
                {translatedCount} / {totalCount} strings
              </span>
            </div>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              {isTranslating ? (
                ...
                <>
                  <Sparkles size={18} />
                  AI Translate ({totalCount - translatedCount} remaining)
                </>
              )}
            </button>
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-white font-mono text-sm bg-white/[0.03] p-2.5 rounded-lg border border-white/[0.06]">
                        {entry.msgid}
                      </p>
                    </div>
                    <div>
                      ...
                      {entry.status === 'translated' ? (
                        <p className="text-emerald-400 font-myanmar text-sm bg-emerald-500/[0.06] p-2.5 rounded-lg border border-emerald-500/10">
                          {entry.msgstr}
                        </p>
                      ) : (
                        <p className="text-white/20 italic text-sm p-2.5">
                          Pending translation
                        </p>
                      )}
                    </div>
      ...
      {step === 'export' && (
        <div className="glass-card p-8 text-center max-w-lg mx-auto">
          ...
          <p className="text-white/50 mb-6">
            {translatedCount} strings translated to{' '}
          ...
          <div className="mt-6 pt-4 border-t border-white/[0.06]">
            <div className="flex items-center justify-center gap-2 text-xs text-white/30">
              <Shield size={12} />
              <span>QA verified with placeholder integrity checks</span>
            </div>
          </div>
        </div>
      )}
